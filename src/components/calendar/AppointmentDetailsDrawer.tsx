...
import type { Appointment } from "@/lib/ui-lab/types";
...
function DetailRow({
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      >
        <span style={{ color: "var(--v2-text-muted)" }}>{icon}</span>
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[11px] uppercase tracking-wider font-semibold mb-0.5" style={{ color: "var(--v2-text-muted)" }}>
          {label}
        </p>
...
function CancelModal({
  ...
  // Reset on open
  useEffect(() => {
    if (open) { setReason(""); setNote(""); setError(""); }
  }, [open]);
...
export function AppointmentDetailsDrawer({
  ...
  // Close cancel modal if drawer closes
  useEffect(() => {
  ...
  const handleCancelConfirm = (reason: string, note: string) => {
    ...
    onCancel(appointment.id, { reason, note: note.trim() || undefined });
    // onCancel in parent will also close the drawer
  };
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 h-10 rounded-[var(--v2-radius-pill)] text-[13px] font-medium border opacity-40 cursor-not-allowed"
              style={{ borderColor: "var(--v2-border)", color: "var(--v2-text)" }}
            >
              Izmeni
            </button>

            {/* Cancel — hidden once already cancelled/completed */}
      ...
      {/* Cancel confirmation modal — rendered at this level so it floats above the drawer */}
      <CancelModal
